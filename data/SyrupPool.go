...
import (
	...
	"github.com/walletConsole/pancakeswap-console/abi/masterchef"
...
func (tp *SyrupPool) CakeSyrupPool() {
	...
	f := 1000 * rewardPerBlock * 20 * 60 * 24 * share / totalAmount * price
	tp.Estimation.Set(p.Sprintf("%.2f", f))
...
func (tp *SyrupPool) OtherSyrupPool() {
	...
	f := 1000 / totalAmount * rewardPerBlock * 20 * 60 * 24 * poolPrice
	tp.Estimation.Set(p.Sprintf("%.2f", f))
